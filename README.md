...
Edit the JCW placeholder lines at `<workspace>/resolved`, then preview it:
...
`source` is the exact source byte snapshot. `resolved` is an ordinary byte file initialized with one self-describing JCW placeholder line per conflict region: each line names its region index and term artifact paths and mirrors the region's line ending. It is not a resolution and does not choose a winner. `apply` refuses the resolved file while any placeholder remains; the agent must replace each `JCW-UNRESOLVED-CONFLICT-REGION-NNN` line with the final content for that conflict, or delete the line to drop the content. Each conflict region is represented by `regions/region-NNN/term-NNN.term`; term files contain the exact logical bytes for the corresponding snapshot term. `manifest.json` records schema/version, canonical source identity, repository-relative path, source SHA-256 and length, marker widths, region ranges, per-region placeholder seed bytes, term labels/kinds, exact lengths, final-newline metadata, digests, and generated artifact paths.

The resolved seed is marker-free (it can never be mistaken for a native conflict marker) and deliberately refuses to install itself: `apply` compares each region's recorded seed bytes against the resolved file and rejects any region that still contains them. Agents may replace a region with empty bytes, delete it, or add bytes, but bytes outside the recorded conflict ranges must remain unchanged.
